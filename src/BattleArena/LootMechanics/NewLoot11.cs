...
public partial class NewLoot11 : FlashNode2D
{
    ...
    public void getLoot()
    {
        if (lootValue == 1)
        {
            ...
            this.GetArena().showDamage("Crystal of Rarity +1", 13369086, _X, _Y - 20);
            if (_root.save.questType == "Loot")
            {
                if (
                    _root.save.questSubtype == "Any"
                    || _root.save.questSubtype == "Crystal of Rarity"
                )
                {
        ...
        else if (lootValue == 2)
        {
            ...
            this.GetArena().showDamage("Crystal of Ultimate Rarity +1", 16698366, _X, _Y - 20);
            if (_root.save.questType == "Loot")
            {
                if (
                    _root.save.questSubtype == "Any"
                    || _root.save.questSubtype == "Crystal of Ultimate Rarity"
                )
                {
                    _root.save.questCount += 1;
                }
    ...
    {
        Arena arenaNode = (Arena)this.GetTree().GetFirstNodeInGroup("arena");
