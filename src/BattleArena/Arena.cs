...
public partial class Arena : SceneData
{
    ...
    public const string newLoot3_sceneId = "uid://bj16q1ilmll5m";
    public const string newLoot4_sceneId = "uid://hhcbuj0ac5rn";
    public const string newLoot5_sceneId = "uid://cewnbti5unski";
    public const string newLoot6_sceneId = "uid://d3nlhqp4h67ha";
    public const string newLoot7_sceneId = "uid://dnp4rssevxw5m";
    public const string newLoot8_sceneId = "uid://cbvgek2ip6j07";
    public const string newLoot9_sceneId = "uid://bf2elwkbxuch5";
    public const string newLoot10_sceneId = "uid://csu27nqapetiu";
    public const string newLoot11_sceneId = "uid://c7b68b1hdny5w";
    public const string newLoot12_sceneId = "uid://15ky8lnb1swy";
    public const string newLoot13_sceneId = "uid://be4fmtn6sxesl";
    public const string newLoot14_sceneId = "uid://x60e6ml2qdhw";
    public const string newLoot15_sceneId = "uid://r8joswlt0chr";
    public const string newLoot16_sceneId = "uid://dfyrrtmkpjrhx";
    public const string newLoot17Frame1_sceneId = "uid://bocpobovhkj68";
    public const string newLoot17Frame2_sceneId = "uid://c05qcymarg38l";
    public const string newLoot18_sceneId = "uid://dgjg8ecqpau87";
    public const string newLoot19_sceneId = "uid://dsmbcwisjv6e2";
    public const string newLoot20_sceneId = "uid://c4mr70ui3ljhm";
    public const string newLoot21_sceneId = "uid://cjuplaukbed27";
    public const string newLoot22_sceneId = "uid://cgpc2t6w3tlus";
                    ...
                    )
                    {
                        ...
                        )
                        {
                            ...
                            if (enemy.zombie == true && enemy.enemyID != 0)
                            {
                                ...
                                {
                                    // NOTE: why did this used to reach into the scene tree dirctly in flash?
    ...
    public void subFunction1()
    {
        ...
        if (_root.spiritUnleash > 0)
        {
            ...
            else if (_root.save.inventorySubtype[_root.save.arenaWeapon] == "Hammer")
            {
                ...
                _root.save.inventorySubtype[_root.save.arenaWeapon] == "Polearm"
                || _root.save.inventorySubtype[_root.save.arenaWeapon] == "Polearm (Stab)"
            )
            {
                _root.abilAttack += 10;
        ...
        {
            _root.abilRageAttack += Math.floor(
                (5 + 0.05 * _root.save.arenaRuneLevel[3]) * runeMult
            );
        ...
        {
            if (
                _root.save.inventoryName[i] == "Perfect Attendance"
                && _root.save.inventoryExist[i] == 1
                && dilStack < 2
                && _root.save.inventoryExpiry[i] > _root.systemtimenow
            ...
            if (
                _root.save.inventoryName[i] == "Mission Kommander\'s Gem"
                && _root.save.inventoryExist[i] == 1
                && komStack < 1
                && _root.save.inventoryDesc[i]
                    != "This medal\'s name depends on your Anti-Idle title when you craft it. Don\'t ask how that works."
            ...
            {
                komStack += 1;
                _root.abilPixel += Math.floor(
                    Math.min(Math.sqrt(_root.save.arenaKommanderComplete), 50)
                );
            }
            i++;
        }
        ...
        _root.allyPassive1X = Math.floor(
            _root.enemyList[_root.save.arenaAlly].allyPassive1X
                * (1 + _root.save.arenaAllyUpgrade[_root.save.arenaAlly] / 10)
        );
        _root.allyPassive2 = _root.enemyList[_root.save.arenaAlly].allyPassive2;
        _root.allyPassive2X = Math.floor(
            _root.enemyList[_root.save.arenaAlly].allyPassive2X
                * (1 + _root.save.arenaAllyUpgrade[_root.save.arenaAlly] / 10)
        );
        ...
        while (i <= 3)
        {
            if ((string)_root["allyPassive" + i] == "Reward")
            {
                _root.allyExp += (double)_root["allyPassive" + i + "X"];
                _root.allyCoin += (double)_root["allyPassive" + i + "X"];
                _root.allyPixel += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "EXP")
            {
                _root.allyExp += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Coin")
            {
                _root.allyCoin += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Pixel")
            {
                _root.allyPixel += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Reflect")
            {
                _root.allyReflect += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Drop Rate")
            {
                _root.allyDropRate += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Spawn Rate")
            {
                _root.allySpawnRate += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Attack %")
            {
                _root.allyAttack += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Defense %")
            {
                _root.allyDefense += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "MaxHP %")
            {
                _root.allyHP += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "MaxMP %")
            {
                _root.allyMP += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Accuracy %")
            {
                _root.allyAccuracy += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Evasion %")
            {
                _root.allyEvasion += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Equipment Attack")
            {
                _root.allyEquipAttack += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Equipment Defense")
            {
                _root.allyEquipDefense += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Attack Speed")
            {
                _root.allyAttackSpeedRaw += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Attack Speed %")
            {
                _root.allyAttackSpeed += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Mastery")
            {
                _root.allyMastery += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Resist")
            {
                _root.allyDamageTaken += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Hit Chance")
            {
                _root.allyHitChance += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Dodge Chance")
            {
                _root.allyDodgeChance += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Critical Chance")
            {
                _root.allyCriticalChance += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Critical Damage")
            {
                _root.allyCriticalDamage += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Magic Resist")
            {
                _root.allyMagicResist += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Double Hit Chance")
            {
                _root.allyDoubleHit += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Instant Kill")
            {
                _root.allyInstantKill += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Boss Damage")
            {
                _root.allyBossDamage += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Non-Boss Damage")
            {
                _root.allyNonBossDamage += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Damage")
            {
                _root.allyNonBossDamage += (double)_root["allyPassive" + i + "X"];
                _root.allyBossDamage += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Ignore Defense")
            {
                _root.allyIgnoreDefense += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Potion Efficiency")
            {
                _root.allyPotionEfficiency += (double)_root["allyPassive" + i + "X"];
            }
            else if ((string)_root["allyPassive" + i] == "Negate Effect")
            {
                _root.allyNegateEffect += (double)_root["allyPassive" + i + "X"];
            }
            i++;
        }
    ...
    public void checkStat()
    {
        ...
        while (i <= TMI)
        {
            ...
            {
                _root.abilIncEquipAttack += Math.ceil(0.15 * itemTier[i] * itemTier[i]);
            }
            else if (itemMoreBonus[i] == "Equipment Defense")
            {
                _root.abilIncEquipDefense += Math.ceil(1 * itemTier[i] * itemTier[i]);
            }
            else if (itemMoreBonus[i] == "Attack Power")
            {
                _root.abilIncAttack += Math.ceil(4 * itemTier[i] * itemTier[i]);
            }
            else if (itemMoreBonus[i] == "Defense Power")
            {
                _root.abilIncDefense += Math.ceil(8 * itemTier[i] * itemTier[i]);
            }
            else if (itemMoreBonus[i] == "MaxHP")
            {
                _root.abilIncHP += Math.ceil(250 * itemTier[i] * itemTier[i]);
            }
            else if (itemMoreBonus[i] == "MaxMP")
            {
                _root.abilIncMP += Math.ceil(100 * itemTier[i] * itemTier[i]);
            }
            else if (itemMoreBonus[i] == "Accuracy")
            {
                _root.abilIncAccuracy += Math.ceil(4 * itemTier[i] * itemTier[i]);
            }
            else if (itemMoreBonus[i] == "Evasion")
            {
                _root.abilIncEvasion += Math.ceil(8 * itemTier[i] * itemTier[i]);
            }
            else if (itemMoreBonus[i] == "EXP / Boss Kill")
            {
                _root.abilIncExpBoss += Math.ceil(2 * itemTier[i] * itemTier[i]);
            }
            else if (itemMoreBonus[i] == "EXP / Rare Kill")
            {
                _root.abilIncExpRare += Math.ceil(2 * itemTier[i] * itemTier[i]);
            }
            else if (itemMoreBonus[i] == "EXP / Epic Kill")
            {
                _root.abilIncExpEpic += Math.ceil(10 * itemTier[i] * itemTier[i]);
            }
            if (i == 1 && _root.spiritUnleash > 0)
            {
                itemTier[1] += 10;
            }
            ...
            else if (itemAbility[i] == "Attack Speed")
            {
                _root.abilAttackSpeed += itemTier[i] * 2;
            }
            else if (itemAbility[i] == "Ignore Defense")
            {
                _root.abilIgnoreDefense += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Mastery")
            {
                _root.abilMastery += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Critical Chance")
            {
                _root.abilCriticalChance += Math.ceil(itemTier[i] * 1.5);
            }
            else if (itemAbility[i] == "Critical Damage")
            {
                _root.abilCriticalDamage += itemTier[i] * 3;
            }
            else if (itemAbility[i] == "Defense")
            {
                _root.abilDefense += itemTier[i] * 2;
            }
            else if (itemAbility[i] == "Damage Taken")
            {
                _root.abilDamageTaken += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Reflect")
            {
                _root.abilReflect += itemTier[i] * 50;
            }
            else if (itemAbility[i] == "Accuracy")
            {
                _root.abilAccuracy += itemTier[i] * 2;
            }
            else if (itemAbility[i] == "Hit Chance")
            {
                _root.abilHitChance += Math.ceil(itemTier[i] * 0.5);
            }
            else if (itemAbility[i] == "Evasion")
            {
                _root.abilEvasion += itemTier[i] * 4;
            }
            else if (itemAbility[i] == "Dodge Chance")
            {
                _root.abilDodgeChance += Math.ceil(itemTier[i] * 0.5);
            }
            else if (itemAbility[i] == "Poison Chance")
            {
                _root.abilPoisonChance += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Weaken Chance")
            {
                _root.abilWeakenChance += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Blind Chance")
            {
                _root.abilBlindChance += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Stun Chance")
            {
                _root.abilStunChance += Math.ceil(itemTier[i] * 0.5);
            }
            else if (itemAbility[i] == "Silence Chance")
            {
                _root.abilSilenceChance += itemTier[i] * 2;
            }
            else if (itemAbility[i] == "HP")
            {
                _root.abilHP += itemTier[i] * 5;
            }
            else if (itemAbility[i] == "MP")
            {
                _root.abilMP += itemTier[i] * 5;
            }
            else if (itemAbility[i] == "HP Regen")
            {
                _root.abilHPRegen += itemTier[i] * 100;
            }
            else if (itemAbility[i] == "MP Regen")
            {
                _root.abilMPRegen += itemTier[i] * 50;
            }
            else if (itemAbility[i] == "Rage Depletion")
            {
                _root.abilRageDepletion += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Drain")
            {
                _root.abilLifeDrain += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Mana Eater")
            {
                _root.abilManaEater += itemTier[i] * 500;
            }
            else if (itemAbility[i] == "Autosteal")
            {
                _root.abilAutoSteal += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Double Hit Chance")
            {
                _root.abilDoubleHit += Math.ceil(itemTier[i] * 2.5);
            }
            else if (itemAbility[i] == "Rage Attack")
            {
                _root.abilRageAttack += Math.ceil(itemTier[i] * 1.5);
            }
            else if (itemAbility[i] == "Rage Speed")
            {
                _root.abilRageSpeed += Math.ceil(itemTier[i] * 2.5);
            }
            else if (itemAbility[i] == "Poison Damage")
            {
                _root.abilPoisonDamage += itemTier[i] * 25;
            }
            else if (itemAbility[i] == "Weaken Effect")
            {
                _root.abilWeakenEffect += Math.ceil(itemTier[i] * 0.5);
            }
            else if (itemAbility[i] == "Blind Effect")
            {
                _root.abilBlindEffect += Math.ceil(itemTier[i] * 0.5);
            }
            else if (itemAbility[i] == "Poison Duration")
            {
                _root.abilPoisonDuration += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Weaken Duration")
            {
                _root.abilWeakenDuration += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Blind Duration")
            {
                _root.abilBlindDuration += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Potion Efficiency")
            {
                _root.abilPotionEfficiency += itemTier[i] * 10;
            }
            else if (itemAbility[i] == "Boss Damage")
            {
                _root.abilBossDamage += itemTier[i] * 2;
            }
            else if (itemAbility[i] == "Magic Resist")
            {
                _root.abilMagicResist += Math.ceil(itemTier[i] * 1.5);
            }
            else if (itemAbility[i] == "Instant Kill")
            {
                _root.abilInstantKill += Math.ceil(itemTier[i] * 0.5);
            }
            else if (itemAbility[i] == "Negate Effect")
            {
                _root.abilNegateEffect += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Non-Boss Damage")
            {
                _root.abilNonBossDamage += itemTier[i] * 2;
            }
            else if (itemAbility[i] == "Damage")
            {
                _root.abilNonBossDamage += Math.ceil(itemTier[i] * 1.5);
                _root.abilBossDamage += Math.ceil(itemTier[i] * 1.5);
            }
            else if (itemAbility[i] == "Extra Combo")
            {
                _root.abilExtraCombo += itemTier[i] * 1;
            }
            else if (itemAbility[i] == "Magic Reflect")
            {
                _root.abilMagicReflect += itemTier[i] * 10;
            }
            else if (itemAbility[i] == "Nullify Death Penalty")
            {
                _root.abilNullifyPenalty += 100;
            }
            else if (itemAbility[i] == "Buff Effect")
            {
                _root.abilBuffEffect += Math.ceil(itemTier[i] * 0.5);
            }
            else if (itemAbility[i] == "All Elements")
            {
            ...
            else if (itemAbility[i] == "Max Spirit")
            {
        ...
        if (_root.maxHealth > 19999999)
        {
            if (_root.manaPower == true)
            {
                _root.maxMana += Math.floor((_root.maxHealth - 19999999) / 50);
                _root.maxSpirit += Math.floor((_root.maxHealth - 19999999) / 500000);
            }
        ...
        while (i <= _root.todayEvent)
        {
            ...
            if (_root.eventList[yy][mm][dd][i] == "2.5x Drop Rate in Battle Arena")
            {
    ...
    public void regen()
    {
        ...
        while (i <= 3)
        {
            ...
            if ((double)_root["allyCooldown" + i] <= 0)
            {
                ...
                var tempActiveX = (double)_root["allyActive" + i + "X"];
                var tempActiveY = (double)_root["allyActive" + i + "Y"];
                var tempActiveZ = (double)_root["allyActive" + i + "Z"];
                ...
                )
                {
                    ...
                    if (enemy.zombie == true && enemy.enemyID != 0)
                    {
                        // NOTE: why did this used to reach into the scene tree dirctly in flash?
                        // _root.house.arena.dealDamage(
                        dealDamage(tempActiveY, 0, "Invisible Ally - Ignore Defense");
                    }
                    _root["allyCooldown" + i] = tempActiveZ;
                }
        ...
        {
            var poisonDamage = Math.floor(_root.save.arenaHealth / 5);
        ...
        if (_root.save.arenaZone != 0)
        {
            ...
            while (i <= 6)
            {
                if (_root.save.arenaRuneDuration[i] > 0)
                {
                    _root.save.arenaRuneDuration[i] -= 1;
                }
                if (_root.save.arenaRuneDuration[i] <= 30 && _root.save.arenaRuneAuto[i] == true)
                {
                    if (i <= 3)
                    {
