...
public partial class Fishing : Control
{
    // MATCH: DefineSprite_7870-frame_1-DoAction.as:checkCount()
    ...
    [Export]
    private Score scoreDisp;
    private double curScoreDisp;
    ...
    // MATCH: DefineSprite_7870-frame_1-DoAction.as
    public override void _EnterTree()
    {
        ...
        {
            // scoreDisp.gotoAndStop(2); // TODO
            scoreDisp.skillText.Text =
                "Skill " + _root.save.fishLevel + " -> " + Math.round(_root.save.fishLevel + 1);
            if (_root.save.fishLevel < 50)
            {
                scoreDisp.skillText.Text = "Skill 50 -> 51";
            }

            scoreDisp.perfectText.Text = _root.withComma(
                _root.save.fishExamPerfectReq - _root.save.fishExamPerfect
            );
            ...
            scoreDisp.streakText.Text =
                _root.withComma(_root.save.fishExamStreakReq - _root.save.fishStreak) + "x";
            if (_root.save.fishExamStreak >= _root.save.fishExamStreakReq)
            {
                scoreDisp.streakText.Text = "0x";
            }

            // scoreDisp.lifeGauge.gotoAndStop(_root.save.fishExamLeft); // TODO
    ...
    public override void _Process(double delta)
    {
        if (_root.save.fishExamLeft >= 0)
        {
            ...
            {
                if (
                    _root.save.fishExamPerfect >= _root.save.fishExamPerfectReq
                    && _root.save.fishExamStreak >= _root.save.fishExamStreakReq
                )
                {
                    if (_root.save.fishLevel < 60)
                    {
                        ...
                        _root.dispNews(
                            120,
                            "Your Fishing Skill is now " + _root.save.fishLevel + "!"
                        );
                        _root.save.fishExamReduce = 0;
                        // fishPop.gotoAndStop(5); TODO
                    }
                }
                else
                {
                    // fishPop.gotoAndStop(6); TODO
                }
        ...
        fishFreq =
            (1 + _root.save.fishLevel * 0.02)
            * (5000 - _root.save.fishFatigue)
            / 5000
            * 0.3
            * (1 + _root.save.fishStreak * 0.1);
        ...
        )
        {
            ...
            else if (chaos1 <= 3 && _root.save.fishStreak >= 5)
            {
                ...
                if (chaos <= 4 && _root.save.fishLevel >= 25 || _root.save.fishRod == 3)
                {
                    _root.fishCurrent = 3;
                    if (
                        _root.save.fishRod == 10
                        && _root.save.fishLeft[3] > _root.save.fishLeft[6]
                        && Math.random() < 0.9
                    )
                    {
                        _root.fishCurrent = 6;
                ...
                else if (chaos <= 20 && _root.save.fishLevel >= 15)
                {
                    ...
                    if (
                        _root.save.fishRod == 10
                        && _root.save.fishLeft[2] > _root.save.fishLeft[5]
                        && Math.random() < 0.9
                    )
                    {
                        _root.fishCurrent = 5;
                ...
                else
                {
                    ...
                    if (
                        _root.save.fishRod == 10
                        && _root.save.fishLeft[1] > _root.save.fishLeft[4]
                        && Math.random() < 0.9
                    )
                    {
                        _root.fishCurrent = 4;
            ...
            else if (chaos1 <= 6 && _root.save.fishStreak >= 4)
            {
                ...
                if (chaos <= 4 && _root.save.fishLevel >= 25 || _root.save.fishRod == 3)
                {
                    ...
                    if (
                        _root.save.fishRod == 10
                        && _root.save.fishLeft[6] > _root.save.fishLeft[3]
                        && Math.random() < 0.7
                    )
                    {
                        _root.fishCurrent = 3;
                ...
                else if (chaos <= 20 && _root.save.fishLevel >= 15)
                {
                    ...
                    if (
                        _root.save.fishRod == 10
                        && _root.save.fishLeft[5] > _root.save.fishLeft[2]
                        && Math.random() < 0.7
                    )
                    {
                        _root.fishCurrent = 2;
                ...
                else
                {
                    ...
                    if (
                        _root.save.fishRod == 10
                        && _root.save.fishLeft[4] > _root.save.fishLeft[1]
                        && Math.random() < 0.7
                    )
                    {
                        _root.fishCurrent = 1;
            ...
            else
            {
                ...
                while (i <= fL)
                {
                    if (ftc < 60)
                    {
                        ...
                        if (
                            ftc >= 50
                            && _root.save.fishStreak < (ftc - 50) * 5
                            && _root.save.fishRod == 8
                        )
                        {
                            chance *= 0.3;
                        ...
                        if (
                            ftc >= 50
                            && _root.save.fishStreak < (ftc - 50) * 3
                            && _root.save.fishRod != 8
                        )
                        {
                            chance *= 0.3;
            ...
            _root.fishCurrentExp = Math.floor(
                _root.fishArray[_root.fishCurrent].fishExp * (1 + _root.curCareerLevel[12] * 0.01)
            );
            ...
            _root.fishAExp = Math.floor(
                _root.fishArray[_root.fishCurrent].fishAExp * (1 + _root.curCareerLevel[12] * 0.005)
            );
            if (_root.save.eliteFisherTime > 0)
            {
