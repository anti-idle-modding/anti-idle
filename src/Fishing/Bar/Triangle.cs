...
public partial class Triangle : Control
{
    ...
    private double dist;
    private double distAllow;
    private double streakBonus;
    private double distPenalty;
    ...
    // MATCH: DefineSprite_7732-frame_1-PlaceObject3_7720_9-CLIPACTIONRECORD onClipEvent(keyDown).as
    public override void _Input(InputEvent @event)
    {
        ...
        if (dist <= distAllow || tolerance >= 1)
        {
            ...
            _root.gainCareerEXP(
                12,
                100 + _root.fishCurrentExp * 3 + streakBonus * 5 + _root.save.fishLevel,
                true
            );
            ...
            if (true)
            {
                _root.dispNews(
                    118,
                    "Fishing Mastery increased! (+"
                        + _root.withComma(_root.fishCurrentExp + streakBonus)
                        + ")"
                );
            }
            expToGain = Math.floor(
        ...
        else if (dist <= 15 && _root.fishSpec != "Manual+")
        {
            ...
            if (true)
            {
                _root.dispNews(
                    118,
                    "Fishing Mastery increased! (+" + _root.withComma(_root.fishCurrentExp) + ")"
                );
            }
            if (_root.fishCurrent == 52)
            {
