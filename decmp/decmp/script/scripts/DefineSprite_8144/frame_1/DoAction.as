function checkLight()
{
   ...
   if(_root.save["tech" + i + "Program"] != "Disabled")
   {
      ...
      if(_root.save["tech" + i + "Program"] == "LolMarket Check")
      {
         ...
         while(k <= 10)
         {
            demandDisp = _root.save.lolDemand[k];
            if(_root.save.lolDemand[k] > 0)
            {
               demandDisp = "+" + demandDisp;
            }
            if(_root.save["tech" + i + "Array"][2] == true && _root.save.lolGems[k] <= 0 && _root.save.lolPrice[k] <= k * _root.save["tech" + i + "NumberA"] && _root.save.lolDemand[k] >= _root.save["tech" + i + "NumberC"] * -1)
            {
               tempDesc += "\n<font color=\'#99FF00\'><b>Lv. " + k + ": " + _root.withComma(_root.save.lolPrice[k]) + " [" + demandDisp + "]</b></font>";
               light = true;
            }
            else if(_root.save["tech" + i + "Array"][3] == true && _root.save.lolGems[k] > 0 && _root.save.lolPrice[k] >= k * _root.save["tech" + i + "NumberB"] && _root.save.lolDemand[k] <= _root.save["tech" + i + "NumberC"])
            {
               tempDesc += "\n<font color=\'#FFFF00\'><b>Lv. " + k + ": " + _root.withComma(_root.save.lolPrice[k]) + " [" + demandDisp + "]</b></font>";
               light = true;
            }
            else if(_root.save.lolGems[k] > 0)
            {
               tempDesc += "\n<font color=\'#FFFFFF\'><b>Lv. " + k + ": " + _root.withComma(_root.save.lolPrice[k]) + " [" + demandDisp + "]</b></font>";
            }
            else
            {
               tempDesc += "\n<font color=\'#999999\'><b>Lv. " + k + ": " + _root.withComma(_root.save.lolPrice[k]) + " [" + demandDisp + "]</b></font>";
            }
            k++;
         }
      }
      if(_root.save["tech" + i + "Program"] == "Garden Check")
      {
         ...
         firstSlot = 0;
         slot = 1;
         while(slot <= _root.save.gardenCapacity)
         {
            if(_root.save.gardenTrees[slot] > 0)
            {
               if(_root.save.gardenRecentTime[slot] + _root.save.gardenHarvestTime[slot] - _root.save.gardenPurchaseTime[slot] - _root.save.gardenExpiryTime[slot] > 0)
               {
                  expireCount += 1;
                  if(firstSlot == 0)
                  {
         ...
         while(slot <= 25)
         {
            if(slot <= _root.save.gardenCapacity)
            {
               if(_root.save.gardenTrees[slot] <= 0)
               {
                  expireCount += 1;
