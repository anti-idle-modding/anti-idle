onClipEvent(enterFrame){
   if(_root.arcadeLife > 0 && _root.arcadeAvoidCount < maxHint)
   {
      ...
      {
         _parent.hint1._x = _X;
         _parent.hint1._alpha = 225 - Math.abs(_Y / 2);
      }
      else if(_Y < -50)
      {
         _parent.hint1._alpha = Math.abs(_Y) - 50;
      }
      else
      {
         _parent.hint1._alpha = 0;
      }
   }
   else
   {
   ...
   if(_root.arcadeAvoidCount >= 0)
   {
      ...
      if(_Y > 300)
      {
         ...
         {
            _root.arcadeScore += Math.ceil(power * 300 * _root.arcadeMultiplier);
         }
