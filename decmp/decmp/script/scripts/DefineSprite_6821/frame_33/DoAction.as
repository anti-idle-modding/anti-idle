...
while(i <= _root.todayEvent)
{
   ...
   {
      _root.gainEventToken(_root.arcadeScore / 100000 * 2);
   }
   i++;
}
...
if(_root.save.questType == "Math Master")
{
...
statText.text = _root.arcadeCorrect;
scoreText.text = _root.withComma(_root.arcadeScore);
if(_root.arcadeScore > 100000 && _root.save.rankedMode == true)
{
   _root.submitScore("[Arcade] Math Master",_root.arcadeScore);
}
if(_root.save.highMath < _root.arcadeScore)
{
   _root.save.highMath = _root.arcadeScore;
}
if(_root.save.todayHighMath < _root.arcadeScore)
{
   _root.save.todayHighMath = _root.arcadeScore;
}
_root.save.totalMath += _root.arcadeScore;
checkGlobal();
if(_root.arcadeScore >= 100000)
{
   medalToGet = Math.floor(Math.floor(_root.arcadeScore / 100000) * 2 * (1 + _root.curCareerLevel[6] * 0.02));
...
if(_root.arcadeScore > 8000000)
{
   fakeScore = 8000000;
}
_root.gainCyborgPoint(Math.floor(fakeScore * 2 / 37500));
baseExp = Math.floor(fakeScore / 270 * _root.arcadeRewardMult);
baseCoin = Math.floor(fakeScore / 360 * _root.arcadeRewardMult);
baseGreenCoin = Math.floor(fakeScore / 60 * _root.arcadeRewardMult);
baseBlueCoin = Math.floor(fakeScore / 30000 * _root.arcadeRewardMult);
_root.gainCareerEXP(6,Math.floor(_root.arcadeScore / 600 * _root.arcadeRewardMult),true);
...
{
   addRankedScore(3,_root.arcadeScore,_root.save.arcadeDifficulty[3]);
   checkGlobal();
   if(_root.save.rankedMath < _root.arcadeScore)
   {
      _root.save.rankedMath = _root.arcadeScore;
   }
}
...
_root.dispNews(97,"[Math Master] Final Score: " + _root.withComma(_root.arcadeScore) + " (+" + _root.withComma(finalExp) + " EXP | +" + _root.withComma(finalCoin) + " Coins)");
