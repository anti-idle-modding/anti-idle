...
statText.text = _root.arcadeDifficulty - 1;
scoreText.text = _root.withComma(_root.arcadeScore);
if(_root.arcadeScore > 100000 && _root.save.rankedMode == true)
{
   _root.submitScore("[Arcade] Pong",_root.arcadeScore);
}
if(_root.save.highPong < _root.arcadeScore)
{
   _root.save.highPong = _root.arcadeScore;
}
if(_root.save.todayHighPong < _root.arcadeScore)
{
   _root.save.todayHighPong = _root.arcadeScore;
}
_root.save.totalPong += _root.arcadeScore;
checkGlobal();
...
{
   addRankedScore(1,_root.arcadeScore,_root.save.arcadeDifficulty[1]);
   checkGlobal();
   if(_root.save.rankedPong < _root.arcadeScore)
   {
      _root.save.rankedPong = _root.arcadeScore;
   }
}
...
_root.dispNews(97,"[Pong] Final Score: " + _root.withComma(_root.arcadeScore) + " (+" + _root.withComma(finalExp) + " EXP | +" + _root.withComma(finalCoin) + " Coins)");
