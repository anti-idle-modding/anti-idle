...
if(_root.save.questType == "Ultimate Avoidance")
{
...
statText.text = _root.arcadeAvoidCount;
scoreText.text = _root.withComma(_root.arcadeScore);
if(_root.arcadeScore > 100000 && _root.save.rankedMode == true)
{
   _root.submitScore("[Arcade] Ultimate Avoidance",_root.arcadeScore);
}
if(_root.save.highAvoidance < _root.arcadeScore)
{
   _root.save.highAvoidance = _root.arcadeScore;
}
if(_root.save.todayHighAvoidance < _root.arcadeScore)
{
   _root.save.todayHighAvoidance = _root.arcadeScore;
}
_root.save.totalAvoidance += _root.arcadeScore;
checkGlobal();
...
_root.gainCyborgPoint(Math.floor(fakeScore * 2 / 12500));
baseExp = Math.floor(fakeScore / 90 * _root.arcadeRewardMult);
baseCoin = Math.floor(fakeScore / 120 * _root.arcadeRewardMult);
baseGreenCoin = Math.floor(fakeScore / 20 * _root.arcadeRewardMult);
baseBlueCoin = Math.floor(fakeScore / 10000 * _root.arcadeRewardMult);
_root.gainCareerEXP(6,Math.floor(_root.arcadeScore / 200 * _root.arcadeRewardMult),true);
...
{
   addRankedScore(2,_root.arcadeScore,_root.save.arcadeDifficulty[2]);
   checkGlobal();
   if(_root.save.rankedAvoidance < _root.arcadeScore)
   {
      _root.save.rankedAvoidance = _root.arcadeScore;
   }
}
...
_root.dispNews(97,"[Ultimate Avoidance] Final Score: " + _root.withComma(_root.arcadeScore) + " (+" + _root.withComma(finalExp) + " EXP | +" + _root.withComma(finalCoin) + " Coins)");
